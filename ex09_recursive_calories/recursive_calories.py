...
def x_sum_loop(nums, x) -> int:
    ...
    if x > 0:
        while 0 <= a <= len(nums):
            return_number += nums[a - 1]
            a += x
    elif x < 0:
        while -len(nums) <= a < 0:
            return_number += nums[a]
            a += x
...
def x_sum_recursion(nums, x) -> int:
    ...
    :return: sum of every x'th number in the list
    """
    if not nums or x > len(nums) or x < -len(nums) or x == 0:
        return 0
    if x > 0:
...
def lets_count_calories(salad: float, chocolate_pieces: int, fridge_visits: int) -> int:
    """
    Every time Kadri goes to fridge, she wants to eat something. In case she has salad in her fridge, she eats exactly.

    100g of it, no matter what. If she has chocolate in the fridge and she had just eaten salad, she takes one piece of
    chocolate. In case she came to fridge and didn't have any salad to eat, she takes two pieces of chocolate (if she
    has at least two pieces, if she doesn't, she takes just one). She keeps on going to the fridge for a little snack
    until she either runs out of fridge visits or snacks.
    ...
    """
    calories = 0
    if not fridge_visits:
        return 0
    else:
        if chocolate_pieces > 0:
            if not salad and chocolate_pieces >= 2:
                calories += 68
                chocolate_pieces -= 2
            else:
                calories += 34
        ...
        fridge_visits -= 1
        return calories + lets_count_calories(salad, chocolate_pieces, fridge_visits)
...
def cycle(cyclists: list, distance: float, time: int = 0, index: int = 0) -> str:
    ...
    showing how long it cycles first. If there are no cyclists or distance is 0 or less, return message "Everyone
    fails." else return the last cyclist to carry others and total time taken to cross the finish line, including the
    last cyclist's "over" minutes: "{cyclist1} is the last leader. Total time: {hours}h {minutes}min."
    ...
    :param cyclists: list on tuples, containing cyclist's name, distance it cycles first and time in minutes how long it
    takes it.
    ...
    """
    index = int(index)
    if not cyclists or not distance:
        ...
        while distance >= 0:
            ...
            rider_time = cyclists[index][2]
            time += rider_time
            if distance >= 0:
                index += 1
            return cycle(cyclists, distance, time, index)
        time = str(time // 60) + "h " + str(time % 60) + "min."
        return cyclists[index][0] + " is the last leader. Total time: " + time
