...
class Order:
    ...
    def add_order_item(self, order_item: OrderItem):
        """
        Add order item to order if not full.
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        for item in self.order_items:
            if customer == item.customer:
                if (cur_cqy + item.quantity) <= max_items_quantity and (cur_vol + item.one_item_volume) <= max_volume:
                    cur_cqy += item.quantity
                    cur_vol += item.total_volume
                    items.append(item)
        for item in items:
...
class ContainerAggregator:
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        for d in containers:
            if not containers[d]:
                empty_containers.append(d)
        for d in empty_containers:
            del containers[d]
        return containers
